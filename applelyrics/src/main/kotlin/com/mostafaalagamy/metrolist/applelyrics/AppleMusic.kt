...
object AppleMusic {
    private val logger = Logger.getLogger(AppleMusic::class.java.name)
    ...
    private suspend inline fun <T> makeRequest(endpoint: String, crossinline block: suspend (HttpClient, URLBuilder) -> T): T? {
        for (baseUrl in baseUrls) {
            ...
            try {
                logger.info("Attempting request to ${urlBuilder.buildString()}")
                val response = block(httpClient, urlBuilder)
                logger.info("Request to ${urlBuilder.buildString()} successful")
                return response
            } catch (e: Exception) {
                logger.warning("Failed request to ${urlBuilder.buildString()}: $e")
            }
        }
        logger.warning("All requests failed for endpoint $endpoint")
        return null
    }
