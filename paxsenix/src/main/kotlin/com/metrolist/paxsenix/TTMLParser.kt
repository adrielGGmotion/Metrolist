...
object TTMLParser {
                                ...
                                when (role) {
                                    ...
                                    "x-translation", "x-roman" -> {
                                        ...
                                        if (wordText.isNotEmpty() && wordBegin.isNotEmpty() && wordEnd.isNotEmpty()) {
                                            val rawText = span.textContent ?: ""
                                            val hasTrailingSpace = rawText.endsWith(" ") || rawText.endsWith("\n") || rawText.endsWith("\u00A0") ||
                                                (node.nextSibling?.nodeType == Node.TEXT_NODE && node.nextSibling.textContent?.contains(Regex("\\s")) == true)
        ...
        for (j in 0 until childNodes.length) {
            ...
            if (node.nodeType == Node.ELEMENT_NODE) {
                ...
                if (innerSpan?.tagName?.lowercase() == "span") {
                    ...
                    if (wordText.isNotEmpty() && wordBegin.isNotEmpty() && wordEnd.isNotEmpty()) {
                        val rawText = innerSpan.textContent ?: ""
                        val hasTrailingSpace = rawText.endsWith(" ") || rawText.endsWith("\n") || rawText.endsWith("\u00A0") ||
                            (node.nextSibling?.nodeType == Node.TEXT_NODE && node.nextSibling.textContent?.contains(Regex("\\s")) == true)
    ...
    private fun parseTime(timeStr: String): Double {
        return try {
            val cleanedTime = timeStr.removeSuffix("s").trim()
            when {
                cleanedTime.contains(":") -> {
                    ...
                    when (parts.size) {
                        ...
                        else -> cleanedTime.toDoubleOrNull() ?: 0.0
                    }
                }
                else -> cleanedTime.toDoubleOrNull() ?: 0.0
            }
